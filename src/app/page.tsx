...
export default function LandingPage() {
  ...
  const [appFeaturesPaused, setAppFeaturesPaused] = useState(false);
      ...
      <section className="relative bg-black text-white py-12 md:py-16 overflow-hidden">
        ...
        <Container className="relative z-10">
          <div className="max-w-6xl mx-auto text-center">
            ...
            <dl className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mt-10 justify-items-center">
              {statistics.map((stat, index) => (
                <div key={index} className="bg-white/10 backdrop-blur-md rounded-lg p-3 border border-white/20 w-full text-center">
                  <stat.icon className="h-6 w-6 text-red-500 mb-2 mx-auto" aria-hidden="true" />
                  <dt className="text-xl md:text-2xl font-bold mb-1">{stat.value}</dt>
                  <dd className="text-xs text-white/80 leading-tight">{stat.label}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <Container>
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-center max-w-7xl mx-auto">
            ...
            <div className="relative">
              ...
              {/* Navigation Dots */}
              <div className="flex items-center justify-center gap-2 mt-6" role="tablist" aria-label="App features gallery">
                ...
                <button
                  onClick={() => setAppFeaturesPaused((p) => !p)}
                  aria-label={appFeaturesPaused ? "Resume app feature auto-rotation" : "Pause app feature auto-rotation"}
                  aria-pressed={appFeaturesPaused}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-gray-300 text-gray-600 text-xs font-medium hover:bg-gray-100 hover:text-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
                >
                  {appFeaturesPaused ? <Play className="h-3.5 w-3.5" aria-hidden="true" /> : <Pause className="h-3.5 w-3.5" aria-hidden="true" />}
                  {appFeaturesPaused ? "Play" : "Pause"}
                </button>
              </div>
            </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    {currentAppFeature === index && (
                      <CheckCircle2 className="h-5 w-5 text-white flex-shrink-0" aria-hidden="true" />
                    )}
                  </div>
                </button>
              ))}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col justify-center gap-3">
                    ...
                    <Button asChild className="bg-black hover:bg-gray-800 text-white">
                      <Link
                        href="https://play.google.com/store/apps/details?id=com.saral_info.moneymaker.sunidhi&hl=en-IN"
                        target="_blank"
                        aria-label="Download the Sunidhi app from Google Play Store (opens in a new tab)"
                      >
                    ...
                    <Button asChild variant="outline" className="border-2 border-gray-300 hover:bg-gray-50">
                      <Link
                        href="https://apps.apple.com/in/app/sunidhi-online/id1553728281"
                        target="_blank"
                        aria-label="Download the Sunidhi app from the Apple App Store (opens in a new tab)"
                      >
                        <svg className="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <Container>
          ...
          <div className="max-w-5xl mx-auto relative">
            <Card className="bg-gradient-to-br from-primary-50 to-white border-2 border-primary-200 shadow-xl">
              <CardContent className="p-8 md:p-12" aria-live="polite" aria-atomic="true">
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-6">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <div className="mb-3">
                      <span className="sr-only">Rated {testimonials[currentTestimonial].rating} out of 5 stars</span>
                      {[...Array(testimonials[currentTestimonial].rating)].map((_, i) => (
                        <Star key={i} className="inline h-5 w-5 text-yellow-400 fill-yellow-400" aria-hidden="true" />
                      ))}
                    </div>
                    <dl>
                      <dt className="sr-only">Client name</dt>
                      <dd className="font-bold text-2xl text-gray-900 mb-1">
                        {testimonials[currentTestimonial].name}
                      </dd>
                      <dt className="sr-only">Client company or role</dt>
                      <dd className="text-gray-600 text-base mb-1">{testimonials[currentTestimonial].company}</dd>
                      {testimonials[currentTestimonial].duration && (
                        <>
                          <dt className="sr-only">Duration of association with Sunidhi</dt>
                          <dd className="text-primary-600 font-semibold text-sm inline-block bg-primary-50 px-3 py-1 rounded-full">
                            {testimonials[currentTestimonial].duration}
                          </dd>
                        </>
                      )}
                ...
                {/* Quote Icon and Text */}
                <div className="relative">
                  <Quote className="h-10 w-10 text-primary-200 mb-3" aria-hidden="true" />
                ...
                <div className="flex items-center justify-center gap-2 mt-8" role="tablist" aria-label="Testimonials">
                  ...
                  <button
                    ...
                    aria-label={testimonialsPaused ? "Resume testimonial auto-rotation" : "Pause testimonial auto-rotation"}
                    aria-pressed={testimonialsPaused}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-gray-300 text-gray-600 text-xs font-medium hover:bg-gray-100 hover:text-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
                  >
                    {testimonialsPaused ? <Play className="h-3.5 w-3.5" aria-hidden="true" /> : <Pause className="h-3.5 w-3.5" aria-hidden="true" />}
