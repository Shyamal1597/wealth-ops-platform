...
import { verifyAdminToken, verifyAdminPermission } from "@/lib/admin-auth";
...
export async function DELETE(request: NextRequest) {
  try {
    // Verify admin authentication + permission (TDL-001 fix)
    const cookieStore = await cookies();
    const token = cookieStore.get("admin-token")?.value;
    ...
    // TDL-001 fix: check permission from the DATABASE, not from client-supplied data
