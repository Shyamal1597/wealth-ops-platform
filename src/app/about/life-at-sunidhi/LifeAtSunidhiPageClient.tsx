...
export default function LifeAtSunidhiPage() {
  ...
  const specialDaysImages = images.filter(img => img.category === 'special-days');
  return (
    <>
      ...
      <section className="py-16">
        <Container>
          <div className="max-w-6xl mx-auto">
            ...
            {/* POSH Policy Section */}
            <div className="bg-white rounded-lg p-8 shadow-md border-l-4 border-primary-600 mb-12">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                  <h3 className="text-2xl font-bold text-gray-900 mb-2">Commitment to Safety (POSH Policy)</h3>
                  <p className="text-gray-700 max-w-2xl">
                    Sunidhi is committed to providing a safe, secure, and dignified work environment for all employees. We have a zero-tolerance policy towards sexual harassment and have established a dedicated Internal Complaints Committee (ICC) to address any concerns.
                  </p>
                </div>
                <a
                  href="/policies/Posh Policy scan.pdf"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Download POSH Policy PDF (opens in a new tab)"
                ...
                >
                  <Trophy className="h-5 w-5" aria-hidden="true" /> {/* Reusing Trophy icon as a generic 'Official' icon or we can import FileText */}
                  Download Policy
                </a>
              </div>
            </div>

            {/* Independence Day Celebration Images */}
            {loading ? (
              <div className="text-center py-8">
                <p className="text-gray-500">Loading celebration images...</p>
              </div>
            ) : specialDaysImages.length > 0 && (
              <div className="mt-8">
                <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Independence Day Celebration</h3>
                <div className="grid md:grid-cols-2 gap-8">
                  {specialDaysImages.map((img) => (
                    <div key={img.id} className="relative h-80 rounded-lg overflow-hidden shadow-xl">
                      <Image
                        src={img.image}
                        alt={img.title}
                        fill
