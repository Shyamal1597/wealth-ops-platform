...
import { Calendar, User, Tag, ArrowRight, Search, Images, Newspaper } from "lucide-react";
...
// Featured/list cards fall back to a placeholder when a post has no image
// (image is optional — a post may be text-only) instead of crashing
// next/image on an empty src.
function BlogThumbnail({ blog, className, children }: { blog: Blog; className: string; children?: React.ReactNode }) {
  return (
              ...
              {featuredBlogs.slice(0, 2).map((blog) => (
                <Link key={blog.id} href={`/blog/${blog.slug}`}>
                  <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer overflow-hidden">
                    <BlogThumbnail blog={blog} className="h-64">
              ...
              {filteredBlogs.map((blog) => (
                <Link key={blog.id} href={`/blog/${blog.slug}`}>
                  <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer overflow-hidden group">
                    <BlogThumbnail blog={blog} className="h-48 overflow-hidden" />
                    <CardHeader>
                      <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
                        <span className="flex items-center gap-1">
                          <Calendar className="h-4 w-4" />
