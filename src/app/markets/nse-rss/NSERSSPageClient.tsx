            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 max-w-2xl mx-auto w-full">
              ...
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    aria-label="Search company name across all feeds"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-white text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-primary-700"
                  />
                </div>
                {searchTerm && (
                  <button
                                ...
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex-1 min-w-0">
                                    <h4 className="font-semibold text-gray-900 mb-1.5 text-base leading-snug hover:text-primary-600 transition-colors">
                                      <a
                                        ...
                                        aria-describedby={`search-desc-${item.id} search-meta-${item.id}`}
                                      >
                                        {item.title}
                                      </a>
                                    </h4>
                                    {item.content && (
                                      <p id={`search-desc-${item.id}`} className="text-gray-500 text-sm mb-2.5 line-clamp-2">
                                        {item.content}
                                      </p>
                                    )}
                                    <div id={`search-meta-${item.id}`} className="flex items-center gap-4 text-xs text-gray-600">
                                  ...
                                  >
                                    <a
                                      ...
                                      aria-label={`View announcement from ${item.title} dated ${formatDate(item.pubDate)} (opens in a new tab)`}
                                    >
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-gray-900 mb-1.5 text-base leading-snug hover:text-primary-600 transition-colors">
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                aria-describedby={`feed-desc-${item.id} feed-meta-${item.id}`}
                              >
                                {item.title}
                              </a>
                            </h3>
                            {item.content && (
                              <p id={`feed-desc-${item.id}`} className="text-gray-500 text-sm mb-2.5 line-clamp-2">
                                {item.content}
                              </p>
                            )}
                            <div id={`feed-meta-${item.id}`} className="flex items-center gap-4 text-xs text-gray-600">
                              <div className="flex items-center gap-1">
                                <Calendar className="h-3 w-3" aria-hidden="true" />
                                {formatDate(item.pubDate)}
                              </div>
