...
export default function MutualFundsPage() {
  ...
  const filteredAMCs = amcList.filter((amc) => {
    ...
    if (!Array.isArray(amc.Schemes)) return false;

    // First check if AMC matches search query
    const matchesSearch = searchQuery === "" ||
      amc.AMC.toLowerCase().includes(searchQuery.toLowerCase()) ||
      amc.Schemes.some(scheme => scheme.toLowerCase().includes(searchQuery.toLowerCase()));

    if (!matchesSearch) return false;

    // If "all" category selected, show all matching AMCs
    if (selectedCategory === "all") return true;

    // Check if AMC has schemes in the selected category
    return amc.Schemes.some(scheme => {
      const categories = getSchemeCategory(scheme);
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-sm text-gray-300">{amc.Count} schemes available</span>
                            </div>
                            <div className="mt-3">
