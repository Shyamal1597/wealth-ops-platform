...
export async function POST(request: NextRequest) {
  try {
    ...
    const reportType = (formData.get('reportType') as string) || 'research';
    ...
    const researchDir = path.join(process.cwd(), 'public', 'research-reports');
    ...
    const filePath = path.join(researchDir, sanitizedFilename);

    await writeFile(filePath, buffer);

    // Determine which data file to use based on report type
    ...
    await writeFile(dataFile, JSON.stringify(outputData, null, 2));
